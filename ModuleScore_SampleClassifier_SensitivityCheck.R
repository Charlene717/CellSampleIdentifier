...
## -------------------------------
## 1) 基因 signature（若你前面已有 signatures 物件，可直接沿用）
## -------------------------------
...
message("Signature sizes: ", paste(names(signatures), vapply(signatures, length, integer(1)), sep="=", collapse="; "))
...
## -------------------------------
# 各 sample 的類別次數
counts_list <- lapply(rownames(res_mat), function(sid) {
  v <- res_mat[sid, ]
...
## -------------------------------
# (a) 穩定度長條圖
p_stab <- ggplot(summary_df, aes(x=reorder(sample_id, stability), y=stability,
                                 fill=agree_with_baseline)) +
  geom_col(width=0.7) +
  coord_flip() +
...
counts_long <- counts_df |>
  select(sample_id, Keloid, Normal, `Post-Tx`) |>
  pivot_longer(-sample_id, names_to="Class", values_to="Count") |>
  group_by(sample_id) |>
